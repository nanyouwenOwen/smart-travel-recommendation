...
class TripFlowIntegrationTest {
    ...
    void completesCreateAdjustVersionRestoreListAndDeleteFlow() throws Exception {
        ...
        MvcResult accepted = mvc.perform(post("/api/v1/trips")
                        .header("Authorization", bearer(access))
                        .header("Idempotency-Key", "create-trip-0001")
                        .contentType(MediaType.APPLICATION_JSON).content(tripBody("成都", 2).replace("3000.00", "50.00")))
        ...
        mvc.perform(post("/api/v1/trips").header("Authorization", bearer(access))
                        .header("Idempotency-Key", "create-trip-0001")
                        .contentType(MediaType.APPLICATION_JSON).content(tripBody("成都", 2).replace("3000.00", "50.00")))
                .andExpect(status().isAccepted()).andExpect(jsonPath("$.data.id").value(tripId));
        mvc.perform(post("/api/v1/trips").header("Authorization", bearer(access))
                        .header("Idempotency-Key", "create-trip-0001")
                        .contentType(MediaType.APPLICATION_JSON).content(tripBody("重庆", 2).replace("3000.00", "50.00")))
                .andExpect(status().isConflict())
                .andExpect(jsonPath("$.error.code").value("IDEMPOTENCY_KEY_CONFLICT"));

        mvc.perform(post("/api/v1/trips/{id}/adjustments", tripId)
        ...
        mvc.perform(post("/api/v1/trips/{id}/versions/{version}:restore", tripId, 1)
                        .header("Authorization", bearer(access)))
                .andExpect(status().isOk()).andExpect(jsonPath("$.data.currentVersion").value(1));
        mvc.perform(get("/api/v1/trips?limit=1").header("Authorization", bearer(access)))
                .andExpect(status().isOk()).andExpect(jsonPath("$.data[0].id").value(tripId))
                .andExpect(jsonPath("$.meta.hasMore").value(false));

        String changed = tripBody("成都和乐山", 3).replace("3000.00", "1.00");
        mvc.perform(patch("/api/v1/trips/{id}", tripId).header("Authorization", bearer(access))
                        .contentType(MediaType.APPLICATION_JSON).content(changed))
                .andExpect(status().isAccepted()).andExpect(jsonPath("$.data.destination").value("成都和乐山"));
        awaitVersion(access, tripId, 3);
        mvc.perform(get("/api/v1/trips/{id}/versions/1", tripId).header("Authorization", bearer(access)))
                .andExpect(status().isOk()).andExpect(jsonPath("$.data.budgetBreakdown.exceedsBudget").value(false));
    ...
    void hidesTripsFromOtherUsersAndValidatesBoundaries() throws Exception {
        ...
        MvcResult result = mvc.perform(post("/api/v1/trips").header("Authorization", bearer(owner))
                        .header("Idempotency-Key", "owner-trip-0001")
                        .contentType(MediaType.APPLICATION_JSON).content(tripBody("西安", 1)))
        ...
        mvc.perform(get("/api/v1/trips/{id}", id).header("Authorization", bearer(stranger)))
                .andExpect(status().isNotFound());
        mvc.perform(patch("/api/v1/trips/{id}", id).header("Authorization", bearer(stranger))
                        .contentType(MediaType.APPLICATION_JSON).content(tripBody("西安", 1)))
                .andExpect(status().isNotFound());
        mvc.perform(delete("/api/v1/trips/{id}", id).header("Authorization", bearer(stranger)))
                .andExpect(status().isNotFound());
        mvc.perform(post("/api/v1/trips/{id}/adjustments", id).header("Authorization", bearer(stranger))
                        .contentType(MediaType.APPLICATION_JSON).content("{\"instruction\":\"增加美食\"}"))
                .andExpect(status().isNotFound());
        mvc.perform(get("/api/v1/trips/{id}/versions", id).header("Authorization", bearer(stranger)))
                .andExpect(status().isNotFound());
        mvc.perform(post("/api/v1/trips/{id}/versions/1:restore", id).header("Authorization", bearer(stranger)))
                .andExpect(status().isNotFound());
        mvc.perform(post("/api/v1/trips").header("Authorization", bearer(owner))
                        .header("Idempotency-Key", "short")
                        .contentType(MediaType.APPLICATION_JSON).content(tripBody("西安", 31)))
        ...
        mvc.perform(post("/api/v1/trips").header("Authorization", bearer(owner))
                        ...
                        .contentType(MediaType.APPLICATION_JSON).content(tripBody("洛阳", 1)))
                .andExpect(status().isAccepted());
        MvcResult page = mvc.perform(get("/api/v1/trips?limit=1").header("Authorization", bearer(owner)))
                .andExpect(status().isOk()).andExpect(jsonPath("$.meta.hasMore").value(true)).andReturn();
        String cursor = json(page).at("/meta/nextCursor").asText();
        mvc.perform(get("/api/v1/trips").param("cursor", cursor + "x")
    ...
    void concurrentIdenticalIdempotentRequestsReuseOneTrip() throws Exception {
        ...
        try (var executor = Executors.newFixedThreadPool(2)) {
            var task = (java.util.concurrent.Callable<MvcResult>) () -> {
                ready.countDown();
                start.await();
                return mvc.perform(post("/api/v1/trips").header("Authorization", bearer(access))
                                .header("Idempotency-Key", "concurrent-create-01")
                                .contentType(MediaType.APPLICATION_JSON).content(tripBody("南京", 1)))
                        .andReturn();
            };
    ...
    private JsonNode awaitStatus(String access, String id, String status) throws Exception {
        ...
        while (Instant.now().isBefore(deadline)) {
            MvcResult result = mvc.perform(get("/api/v1/trips/{id}", id).header("Authorization", bearer(access)))
    ...
    private JsonNode awaitVersion(String access, String id, int version) throws Exception {
        Instant deadline = Instant.now().plus(Duration.ofSeconds(5));
        while (Instant.now().isBefore(deadline)) {
            MvcResult result = mvc.perform(get("/api/v1/trips/{id}/versions/{version}", id, version)
                            .header("Authorization", bearer(access))).andReturn();
    ...
    private String register(String prefix) throws Exception {
        ...
        MvcResult result = mvc.perform(post("/api/v1/auth/register").contentType(MediaType.APPLICATION_JSON)
                        .content("{\"email\":\"" + email + "\",\"password\":\"secure-pass-123\",\"displayName\":\"User\"}"))
    ...
    private String bearer(String access) { return "Bearer " + access; }

    private String tripBody(String destination, int days) {
        return "{\"destination\":\"" + destination + "\",\"startDate\":\"2030-06-01\","
                + "\"days\":" + days + ",\"budget\":{\"amount\":\"3000.00\",\"currency\":\"CNY\"},"

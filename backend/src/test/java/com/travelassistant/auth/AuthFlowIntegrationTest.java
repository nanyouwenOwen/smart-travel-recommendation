...
class AuthFlowIntegrationTest {
    ...
    void completesRegistrationProfileRefreshAndLogoutFlow() throws Exception {
        MvcResult registration = mvc.perform(post("/api/v1/auth/register")
                        ...
                        .content("{\"email\":\" Traveler@Example.com \",\"password\":\"secure-pass-123\",\"displayName\":\" 旅行者 \"}"))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.data.tokenType").value("Bearer"))
                .andReturn();
        JsonNode registered = mapper.readTree(registration.getResponse().getContentAsString()).get("data");
        ...
        mvc.perform(get("/api/v1/users/me").header("Authorization", "Bearer " + access))
                .andExpect(status().isOk()).andExpect(jsonPath("$.data.displayName").value("旅行者"));
        mvc.perform(patch("/api/v1/users/me").header("Authorization", "Bearer " + access)
                        .contentType(MediaType.APPLICATION_JSON).content("{\"displayName\":\"探险家\"}"))
                .andExpect(status().isOk()).andExpect(jsonPath("$.data.displayName").value("探险家"));

        MvcResult rotated = mvc.perform(post("/api/v1/auth/refresh").contentType(MediaType.APPLICATION_JSON)
                        .content("{\"refreshToken\":\"" + refresh + "\"}"))
                .andExpect(status().isOk()).andReturn();
        String nextRefresh = mapper.readTree(rotated.getResponse().getContentAsString())
    ...
    void rejectsDuplicateInvalidLoginAndMissingBearerWithStableErrors() throws Exception {
        String body="{\"email\":\"same@example.com\",\"password\":\"secure-pass-123\",\"displayName\":\"User\"}";
    ...
    void rejectsSoftDeletedUsersTamperedTokensAndOversizedUtf8Passwords() throws Exception {
        String body="{\"email\":\"deleted@example.com\",\"password\":\"secure-pass-123\",\"displayName\":\"User\"}";
        MvcResult result=mvc.perform(post("/api/v1/auth/register").contentType(MediaType.APPLICATION_JSON).content(body))
        ...
        user.softDelete(); users.saveAndFlush(user);
        ...
        mvc.perform(post("/api/v1/auth/register").contentType(MediaType.APPLICATION_JSON)
                        .content("{\"email\":\"bytes@example.com\",\"password\":\"" + longUtf8 + "\",\"displayName\":\"User\"}"))
    ...
    void serializesConcurrentRefreshAndDetectsReplay() throws Exception {
        MvcResult registration=mvc.perform(post("/api/v1/auth/register").contentType(MediaType.APPLICATION_JSON)
                        .content("{\"email\":\"race@example.com\",\"password\":\"secure-pass-123\",\"displayName\":\"Race\"}"))
                .andExpect(status().isCreated()).andReturn();
        String refresh=mapper.readTree(registration.getResponse().getContentAsString())
                .get("data").get("refreshToken").asText();
    ...
    private String jwt(String issuer, String subject, Instant expiry) {
        JwtClaimsSet claims=JwtClaimsSet.builder().issuer(issuer).subject(subject)
                .issuedAt(Instant.now().minusSeconds(120)).expiresAt(expiry).id(UUID.randomUUID().toString()).build();
    ...
    void allowsOnlyOneConcurrentRegistrationForSameEmail() throws Exception {
        String body="{\"email\":\"parallel@example.com\",\"password\":\"secure-pass-123\",\"displayName\":\"Parallel\"}";
        CountDownLatch ready=new CountDownLatch(2); CountDownLatch start=new CountDownLatch(1);
        try (var executor=Executors.newFixedThreadPool(2)) {
            var task=(java.util.concurrent.Callable<Integer>) () -> {
                ready.countDown(); start.await();

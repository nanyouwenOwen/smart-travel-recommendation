...
public class TokenService {
    ...
    public TokenService(JwtEncoder jwtEncoder, RefreshTokenRepository tokens, AuthProperties properties, Clock clock) {
        this.jwtEncoder = jwtEncoder; this.tokens = tokens; this.properties = properties; this.clock = clock;
    }
    ...
    public TokenPair rotate(String rawToken) {
        ...
        if (current.isRevoked()) {
            if (current.getReplacedByTokenId() != null) {
                tokens.findByFamilyIdAndRevokedAtIsNull(current.getFamilyId())
    ...
    private TokenPair createPair(User user, String familyId) {
        ...
        JwtClaimsSet claims = JwtClaimsSet.builder().issuer(properties.issuer()).subject(user.getId())
                .issuedAt(now).expiresAt(expiry).id(UUID.randomUUID().toString()).build();
        JwsHeader header = JwsHeader.with(MacAlgorithm.HS256).build();
        String access = jwtEncoder.encode(JwtEncoderParameters.from(header, claims)).getTokenValue();
        String refresh = randomToken();
        tokens.saveAndFlush(new RefreshToken(user, hash(refresh), familyId, now.plus(properties.refreshTokenTtl())));

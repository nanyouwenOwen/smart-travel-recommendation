...
public class AuthService {
    ...
    public TokenPair register(RegisterRequest request) {
        ...
        } catch (DataIntegrityViolationException e) { throw duplicateEmail(); }
    }
    @Transactional(readOnly=true)
    public User authenticate(LoginRequest request) {
        User user = users.findByEmailIgnoreCase(normalizeEmail(request.email())).orElse(null);
        if (user == null || request.password().getBytes(StandardCharsets.UTF_8).length > 72
    ...
    public TokenPair login(LoginRequest request) { return tokens.issue(authenticate(request)); }
    ...
    public static String normalizeName(String name) {
        ...
        if (normalized.isEmpty()) throw new BusinessException("VALIDATION_ERROR", "展示名称不能为空", HttpStatus.BAD_REQUEST);
        return normalized;
    }
    ...
    private void validatePasswordBytes(String password) {
        if (password.getBytes(StandardCharsets.UTF_8).length > 72)
            throw new BusinessException("VALIDATION_ERROR", "密码 UTF-8 编码后不能超过 72 字节", HttpStatus.BAD_REQUEST);
    }
}

...
public class GlobalExceptionHandler {
  ...
  ResponseEntity<ApiErrorResponse> validation(
    ...
    return response(
        HttpStatus.BAD_REQUEST, new ApiError("VALIDATION_ERROR", "请求参数不合法", details), request);
  }
  ...
  ResponseEntity<ApiErrorResponse> malformedJson(
    ...
    return response(
        HttpStatus.BAD_REQUEST, new ApiError("MALFORMED_JSON", "请求内容不是有效的 JSON"), request);
  }
  ...
  ResponseEntity<ApiErrorResponse> business(
    ...
    return response(
        exception.getStatus(), new ApiError(exception.getCode(), exception.getMessage()), request);
  }

  @ExceptionHandler(UnsafeContentException.class)
  ResponseEntity<ApiErrorResponse> unsafeContent(
      UnsafeContentException exception, HttpServletRequest request) {
    return response(
        HttpStatus.BAD_REQUEST, new ApiError(exception.getCode(), exception.getMessage()), request);
  }
  ...
  ResponseEntity<ApiErrorResponse> unexpected(Exception exception, HttpServletRequest request) {
    ...
    return response(
        HttpStatus.INTERNAL_SERVER_ERROR, new ApiError("INTERNAL_ERROR", "服务暂时不可用"), request);
  }
  ...
  private ResponseEntity<ApiErrorResponse> response(
      HttpStatus status, ApiError error, HttpServletRequest request) {

...
public class CurrentUserService {
  ...
  public UserProfile get(String id) {
    return UserProfile.from(require(id));
  }

  @Transactional
  public UserProfile update(String id, UpdateCurrentUserRequest request) {
    User user = require(id);
    user.changeDisplayName(AuthService.normalizeName(request.displayName()));
  ...
  private User require(String id) {
    return users
        .findById(id)
        .orElseThrow(
            () -> new BusinessException("UNAUTHORIZED", "需要有效的身份认证", HttpStatus.UNAUTHORIZED));
  }
}

...
public class CurrentUserController {
    ...
    public CurrentUserController(CurrentUserService service) { this.service=service; }
    @GetMapping ApiResponse<UserProfile> get(Authentication authentication, HttpServletRequest request) {
        return ApiResponse.of(service.get(authentication.getName()), id(request));
    }
    @PatchMapping ApiResponse<UserProfile> update(Authentication authentication,
            @Valid @RequestBody UpdateCurrentUserRequest body, HttpServletRequest request) {

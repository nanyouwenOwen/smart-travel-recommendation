...
public class TripAsyncConfiguration {
  ...
  ThreadPoolTaskExecutor tripPlanningExecutor(
      @Value("${app.trip-planning.executor.core-size:2}") int core,
      @Value("${app.trip-planning.executor.max-size:4}") int max,
      @Value("${app.trip-planning.executor.queue-capacity:50}") int capacity) {
    ThreadPoolTaskExecutor e = new ThreadPoolTaskExecutor();
    e.setCorePoolSize(core);
    e.setMaxPoolSize(max);
    e.setQueueCapacity(capacity);
    e.setThreadNamePrefix("trip-planner-");

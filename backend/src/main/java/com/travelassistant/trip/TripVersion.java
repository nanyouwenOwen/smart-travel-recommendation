...
@Table(
    name = "trip_versions",
    uniqueConstraints = @UniqueConstraint(columnNames = {"trip_id", "version_number"}))
public class TripVersion {
  ...
  @OneToMany(mappedBy = "tripVersion", cascade = CascadeType.ALL, orphanRemoval = true)
  @OrderBy("dayNumber")
  ...
  public void setAdjustmentInstruction(String instruction) {
    this.adjustmentInstruction = instruction;
  }

  public void setGenerationDurationMs(long duration) {
    this.generationDurationMs = duration;
  }
  ...
  public List<ItineraryDay> getItineraryDays() {
    return List.copyOf(itineraryDays);
  }

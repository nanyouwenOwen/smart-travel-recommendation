...
public class SecurityConfiguration {
  ...
  JwtDecoder jwtDecoder(SecretKey key, AuthProperties p) {
    ...
    decoder.setJwtValidator(
        org.springframework.security.oauth2.jwt.JwtValidators.createDefaultWithIssuer(p.issuer()));
    return decoder;
  }
        ...
        .exceptionHandling(
            e ->
                e.authenticationEntryPoint(
                        (req, res, ex) -> errors.write(req, res, 401, "UNAUTHORIZED", "需要有效的身份认证"))
                    .accessDeniedHandler(
                        (req, res, ex) -> errors.write(req, res, 403, "FORBIDDEN", "无权执行此操作")))

...
@Entity
@Table(name = "conversation_streams")
public class ConversationStream {
  ...
  public ConversationStream(Conversation c, Message m, Instant expiry) {
    conversation = c;
    assistantMessage = m;
    expiresAt = expiry;
  ...
  public void connected() {
    if (status == StreamStatus.DISCONNECTED) status = StreamStatus.ACTIVE;
  }

  public void disconnected() {
    if (status == StreamStatus.ACTIVE) status = StreamStatus.DISCONNECTED;
  }
  ...
  public void fail(StreamStatus s) {
    if (!terminal()) {
      status = s;
      completedAt = Instant.now();
    }

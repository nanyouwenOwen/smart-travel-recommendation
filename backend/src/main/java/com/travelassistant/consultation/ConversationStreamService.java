...
public class ConversationStreamService {
    ...
    public StartedStream start(String user,String conversation,String key,String content){
        ...
        if(preparedStream.replayed())return new StartedStream(preparedStream.streamId(),replay(user,conversation,preparedStream.streamId(),0));
        ...
        executor.submit(()->produce(runtime));return new StartedStream(runtime.id,emitter);
    }
    ...
    private void produce(RuntimeStream runtime){StringBuilder answer=new StringBuilder(),safeBuffer=new StringBuilder();try{
        publish(runtime,StreamEventType.ACK,Map.of("streamId",runtime.id,"userMessageId",runtime.prepared.userMessageId(),"assistantMessageId",runtime.prepared.assistantMessageId(),"eventId",1));flushHeaders(runtime);
    ...
    private void emitDelta(RuntimeStream runtime,String content){publish(runtime,StreamEventType.DELTA,Map.of("streamId",runtime.id,"messageId",runtime.prepared.assistantMessageId(),"content",content));}
    ...
    private void flushHeaders(RuntimeStream runtime){String padding=" ".repeat(2048);runtime.emitters.forEach(e->{try{e.send(SseEmitter.event().comment(padding));}catch(Exception x){detach(runtime,e);}});}
    private ConversationStreamEvent event(String id,StreamEventType type,Object payload){return tx.execute(s->{ConversationStream stream=streams.lockById(id).orElseThrow();try{int sequence=stream.nextSequence();Object persisted=payload;if(type==StreamEventType.DELTA&&payload instanceof Map<?,?>values){Map<String,Object>withSequence=new LinkedHashMap<>();values.forEach((key,value)->withSequence.put(String.valueOf(key),value));withSequence.put("sequence",sequence);persisted=withSequence;}return events.save(new ConversationStreamEvent(stream,sequence,type,mapper.writeValueAsString(persisted),stream.getExpiresAt()));}catch(Exception e){throw new IllegalStateException(e);}});}

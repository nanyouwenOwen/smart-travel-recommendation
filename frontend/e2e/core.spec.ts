...
test('register, plan, version and AI consultation work against the real API', async ({ page }) => {
  ...
  await page.reload()
  await expect(page.getByRole('heading', { name: '我的行程' })).toBeVisible()
  await page.getByRole('link', { name: /新建规划/ }).first().click()
  await page.getByLabel('目的地').fill('京都')
  await page.getByLabel('开始日期').fill(new Date(Date.now() + 86_400_000).toISOString().slice(0, 10))
  ...
  await page.getByLabel('输入旅游问题').fill('请给我一条交通建议')
  await page.getByRole('button', { name: '发送' }).click()
  await expect(page.getByText(/非实时|交通/).last()).toBeVisible({ timeout: 30_000 })
  await page.reload()
  await expect(page.getByText('请给我一条交通建议', { exact: true })).toBeVisible()
})

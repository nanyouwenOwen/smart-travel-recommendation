...
- 支持显式地点检索、目的地地图、行程天气与附近景点营业信息，并展示来源和更新时间。
...
行程生成与实时数据默认使用确定性的 Stub，便于 Codespaces 和本地环境在没有密钥或稳定公网时完整演示。实时数据可通过 `REALTIME_PROVIDER_MODE=stub|live|disabled` 切换；live 模式使用 Nominatim、Open-Meteo 与 Overpass 公共端点，只适合低流量非商业演示，生产环境必须复核政策并换用获批、自托管或商业端点。地图数据 © OpenStreetMap contributors；天气数据归属 Open-Meteo。AI 回答只有在消息携带结构化来源时才表示实际使用了实时数据。

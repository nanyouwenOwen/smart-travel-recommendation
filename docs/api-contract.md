...
- `GET /locations/search?q=&language=zh-CN&limit=5` 仅在用户显式提交后调用，需 Bearer 认证；返回内部 `locationId` 和 OSM 来源。查询长度 2–120、结果上限 10。
- 创建/更新行程可传可选 `destinationLocationId`。服务端只绑定仍有效的内部地点引用，不接受浏览器坐标；旧客户端保持兼容。
- `GET /trips/{tripId}/realtime/weather` 和 `/realtime/places?category=ATTRACTION&limit=10` 均校验行程归属，其他用户统一 404；未绑定地点为 `409 TRIP_LOCATION_REQUIRED`。
- 响应包含结构化 `sources`，区分 `retrievedAt`、可选 `sourceUpdatedAt` 和 `FRESH|STALE|UNAVAILABLE`。供应商故障有缓存时返回 200/STALE 和 warning，无缓存返回稳定数据不可用错误。
- 景点 `openingHours` 是 OSM 原始规则，缺失即 `null`，不得推断为全天营业；website 仅保留 http/https。
- 绑定行程的咨询仅在问题涉及天气或景点营业时按需读取实时数据。服务端把实际使用的 `sources`、`dataUpdatedAt` 和 `freshness` 持久化到 AI 消息；模型文本不能自行声明来源，SSE 完成后以消息回源结果为准。
- `REALTIME_PROVIDER_MODE=stub|live|disabled`。CI 默认 stub，不依赖公网；Transitous 仅保留默认关闭的 provider 边界，本轮不公开路线 API。
...
所有金额由服务端按活动费用重新汇总，分类使用稳定枚举。AI 生成的价格、班次和未携带结构化 `sources` 的营业信息均为一般建议；只有消息或实时接口返回的来源元数据代表本轮实际使用了外部数据。

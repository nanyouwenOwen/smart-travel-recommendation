...
run `29178048659` 已通过完整质量链、candidate、recovery 只读校验和八项逐项上传，失败仍处于复合 `validate-uploaded-release`。本次 diff 只在既有 fail-closed 调用之前设置固定 detail：`load-by-id`、`identity`、`metadata`、`draft-state`、`asset-set`、`asset-downloads`，以及公开后对应的 `published-state`；没有新增、删除、重排任何查询、断言、上传、下载、PATCH 或验证调用。

所有 detail 都是脚本固定字面量，不包含响应、token、文件内容或外部输入，不存在 workflow command 注入来源。阶段与 detail 在潜在失败调用前设置，能把固定 ID 查询、identity、metadata、状态、附件集合和远端下载验证准确区分；成功路径的控制流与发布条件完全不变。

验证：相关 shell `bash -n` PASS；完整发布状态机与既有身份/分类/重试测试 PASS；`git diff --check` PASS。AI log 准确记录 run、八项上传成功、当前复合阶段证据不足和“先诊断、不猜测、不放宽”的决定。

允许推送该诊断提交。下一准确 run 仍必须重新通过完整质量链；其固定子阶段只用于确定后续行动，不能被当作 Release 成功证据。

## identity 纯诊断细分审核

审核结论：**PASS（允许推送 identity 纯诊断细分）**

run `29178200671` 已把上传后失败定位到固定 ID 响应的 `identity` 复验。本次 diff 仅在四个既有断言前依次设置固定 `identity-tag`、`identity-id-type`、`identity-id-continuity`、`identity-upload-url`；tag 比较、正整数条件、expected ID 连续性和精确 upload URL 表达式本身没有任何变化。

所有 detail 均为固定字面量，无响应、token 或外部输入；控制流、失败状态、查询、上传、验证和公开条件完全不变。`bash -n`、完整发布状态机及 `git diff --check` 均 PASS；AI log 准确记录 run 与只诊断、不猜测字段差异的边界。

允许推送。下一准确 run 的子阶段只能用于识别 REST 字段差异，仍不构成发布成功证据。
